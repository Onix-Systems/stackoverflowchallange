...
def make_request(app, user_id, page=1):
    ...
    uri = "{api_url}/2.2/users/{user_id}/posts?key={api_key}&page={page}" \
          "&pagesize=10&order=desc&sort=activity" \
          "&site=stackoverflow&filter=!b0OfN629t08FFj"
    uri = uri.format(api_url=app.config['STACKEXCHANGE_BASE_URL'],
                     api_key=app.config['STACKEXCHANGE_API_KEY'],
...
def build_auth_url(app):
    url = "{base_url}?client_id={client_id}" \
...
def make_post_auth_url(app, code):
    error = None
    data = None
    url = "{base_url}/access_token/json?client_id={client_id}" \
...
def make_get_user_request(app, access_token):
    error = None
    user_id = None
    uri = "{api_url}/2.2/me/associated?key={api_key}" \
